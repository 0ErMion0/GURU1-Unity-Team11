...
public class InfiniteButton : MonoBehaviour
{
    ...
    void Start()        //StageTwo/StageThree 씬에 있는 오브젝트 소속
    {
        ...
        if (nextStage > PlayerPrefs.GetInt("InfiniteAct"))                 //nextStage(25/26)가 StageAct(첫 실행에 24)보다 클 경우 
            PlayerPrefs.SetInt("InfiniteAct", nextStage);                  //StageAct를 nextStage로 바꿈 (PlayerPrefs 사용했으므로 값 유지) 
